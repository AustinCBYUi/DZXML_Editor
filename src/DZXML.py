...
file_to_edit = Path("types.xml")
#file_to_edit = Path(r"Projects/DZXML_Editor/types/chernarus/types.xml") #Dev
...
def quantmin_modifier(change_value):
...
#-------- Quantmax Modifier ---------#
def quantmax_modifier(change_value):
...
#Function created for UI script.
def write_file():
    minimum_check()
    ...
    if minimum_check() == False:
        ...
        tree.write(Path(r"bin\types_modified.xml"))
        print("\033[0;36;48mSuccess")
        # tree.write(r"Projects\DZXML_Editor\bin\types_modified.xml")
